        ...
        private static void Load()
        {
            try
            {
                ...
                foreach (string line in File.ReadAllLines(SettingsPath))
                {
                    ...
                    // Чужие и устаревшие ключи при чтении не отбрасываются и сохраняются
                    // обратно как есть: файл общий, и настройка, которую эта версия ещё
                    // (или уже) не знает, не должна пропадать после запуска.
                }
            }
            catch
            {
