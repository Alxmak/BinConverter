...
# Размеры внутри .ico. Смысл длинного списка: Windows никогда не должна ничего
# масштабировать. Она запрашивает иконку в конкретном числе пикселей, и если
# точного кадра нет — берёт ближайший и растягивает, а растянутый кадр выглядит
# мыльным. Поэтому здесь лежат все размеры, которые запрашивает оболочка:
#   16/20/24 — панель задач и мелкие списки (24 — панель при 150%)
#   32/40/48 — Проводник, рабочий стол (40 — это 32 при 125%, 48 — при 150%)
#   60/64/72/80/96 — крупные значки и те же размеры при 150% и 200%
#   128/256 — плитки, «огромные значки», высокий DPI
ICO_SIZES = [16, 20, 24, 32, 40, 48, 60, 64, 72, 80, 96, 128, 256]
