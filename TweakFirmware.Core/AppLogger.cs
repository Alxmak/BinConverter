...
namespace TweakFirmware.Core
{
    ...
    public static class AppLogger
    {
        ...
        public static void Log(string message)
        {
            ...
            // Подписчик тоже под защитой, и это не перестраховка. Единственный из них
            // (LogService) зовёт Dispatcher.BeginInvoke, а тот бросает исключение, если
            // диспетчер уже начал выключаться, — то есть ровно при закрытии программы
            // во время работы. А уборка за прерванной операцией начинается со строки
            // в журнал (IncompleteOutput.Remove) и идёт внутри обработчика другой ошибки:
            // исключение отсюда отменило бы уборку, оставило на диске недописанные файлы
            // и подменило собой настоящую причину сбоя.
            try
            {
            ...
            {
                // Молча по той же причине, что и запись в файл: пожаловаться на сломанный
                // журнал можно было бы только… через журнал.
            }
        }

        /// <summary>
        /// Пишет только в файл, минуя журнал на экране.
        ///
        /// Нужно для сообщений о необработанных исключениях. Показ строки в журнале сам
        /// способен вызвать исключение (список на экране — обычный элемент интерфейса),
        /// и тогда сообщение об ошибке становится причиной следующей ошибки. Один раз это
        /// уже случилось: программа ушла в бесконечный круг на 720 записей в секунду,
        /// заняла поток интерфейса и выглядела намертво зависшей, хотя операция при этом
        /// дошла до конца.
        /// </summary>
        public static void LogToFileOnly(string message) => TryAppend($"{FormatTimestamp()} {message}");
