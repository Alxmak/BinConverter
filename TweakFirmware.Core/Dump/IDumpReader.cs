...
namespace TweakFirmware.Core.Dump
{
    ...
    public interface IDumpReader : IDisposable
    {
        ...
        /// <summary>
        /// Читает байты по физическому адресу — так, как они лежат в файле, вместе со
        /// служебными областями.
        ///
        /// Нужно ровно одному детектору: у части плееров Blu-ray раскладку страницы
        /// приходится вычислять по самому дампу, а для этого надо видеть служебные байты,
        /// которые обычное чтение как раз и прячет. Всем остальным этот метод не нужен —
        /// работать с дампом следует в логических адресах.
        /// </summary>
        int ReadPhysical(long physicalOffset, Span<byte> destination);
    }
