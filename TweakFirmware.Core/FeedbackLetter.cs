...
namespace TweakFirmware.Core
{
    ...
    public static class FeedbackLetter
    {
        ...
        /// подходит к пределу быстро. Если не помещается, вызывающему коду надо отдать
        /// текст через буфер обмена, а не молча терять его хвост.
        ///
        /// Сейчас программа открывает письмо без текста (только версия и система),
        /// и до предела ему далеко. Проверка остаётся здесь: как только в ссылку снова
        /// начнут класть чей-то текст, обрезание вернётся, а заметно оно не сразу.
        /// </summary>
