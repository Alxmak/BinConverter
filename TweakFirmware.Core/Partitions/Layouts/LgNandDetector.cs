...
namespace TweakFirmware.Core.Partitions.Layouts
{
    ...
    public sealed class LgNandDetector : ILayoutDetector
    {
        ...
        private void ReadBadBlockTable(PartitionTable table, IDumpReader dump, DumpContext context, IAnalysisHost host)
        {
            ...
            // Записей в таблице на одну больше, чем указано в счётчике: даже когда плохих
            // блоков нет, одна запись всё равно есть — по её полю «куда» вычисляется объём
            // микросхемы. Плохими блоками считаются первые count записей, ровно те, что
            // оригинал выводил в журнал. Раскладывал по разделам он при этом все count + 1,
            // то есть засчитывал лишний блок; здесь оба места считают одинаково.
