...
namespace TweakFirmware.Core.Partitions.Layouts
{
    ...
    public static class GptReader
    {
        ...
        /// <summary>
        /// Имя раздела: 72 байта UTF-16 по адресу 0x38, то есть 36 знаков.
        ///
        /// Оригинал читал отсюда 36 байт — цифру из описания формата, где она означает
        /// число знаков, а не байт. У имён в кодировке UTF-16 каждый второй байт нулевой
        /// и отсеивается как непечатаемый, поэтому от имени оставалась ровно первая
        /// половина: восемнадцать знаков. Для «system» и «userdata» разницы нет, а имя
        /// подлиннее обрезалось посередине — и с ним обрезалось имя извлечённого файла.
        ///
        /// Отбор печатаемых символов оставлен как был: имена разделов в прошивках
        /// латинские, а имя отсюда попадает в имя файла на диске.
        /// </summary>
        private static string ReadName(ReadOnlySpan<byte> record)
        {
            const int NameOffset = 0x38;
            const int NameLength = 72;

            var text = new StringBuilder(36);
            for (int i = 0; i < NameLength && NameOffset + i < record.Length; i++)
            {
                byte b = record[NameOffset + i];
