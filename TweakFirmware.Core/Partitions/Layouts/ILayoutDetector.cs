...
namespace TweakFirmware.Core.Partitions.Layouts
{
    ...
    /// повторяет порядок проверок в исходном скрипте, где узкие и надёжные признаки
    /// проверяются раньше широких, и первое же совпадение останавливает перебор.
    /// Переставить два детектора местами — значит изменить, какая разметка выиграет
    /// на дампе, подходящем под обе.
    /// </summary>
    public interface ILayoutDetector
    {
        ...
        /// <summary>
        /// Место детектора в цепочке; меньше — раньше.
        ///
        /// Перебор идёт по порядку списка, а не по этому числу: списки в
        /// <c>PartitionAnalysisOperation</c> написаны руками и повторяют порядок
        /// оригинала. Приоритет — объявленное намерение, и тест проверяет, что списки
        /// ему не противоречат; так случайная перестановка в списке не пройдёт молча.
        /// </summary>
        int Priority { get; }
